...
class VoxelGrid(nn.Module):
    ...
    def coords_to_bounding_voxel_grid(self, coords, coord_features=None,
        ...
        if coord_bounds is not None:
            ...
            res = res[:batch_size]
            bb_mins = bb_mins[:batch_size]
            voxel_indicy_denmominator = voxel_indicy_denmominator[:batch_size]
        ...
        if batch_size == self._batch_size:
            tiled_batch_indices = self._tiled_batch_indices
            ones_max_coords = self._ones_max_coords
            index_grid = self._index_grid
        elif batch_size < self._batch_size:
            tiled_batch_indices = self._tiled_batch_indices[:batch_size]
            ones_max_coords = self._ones_max_coords[:batch_size]
            index_grid = self._index_grid[:batch_size]
        else:
            batch_indices = torch.arange(batch_size, dtype=torch.int, device=coords.device).view(batch_size, 1, 1)
            tiled_batch_indices = batch_indices.repeat([1, self._num_coords, 1])
            ones_max_coords = torch.ones((batch_size, self._num_coords, 1),
                                         device=coords.device, dtype=self._ones_max_coords.dtype)
            w = self._voxel_shape[0] + 2
            arange = torch.arange(0, w, dtype=torch.float, device=coords.device)
            index_grid = torch.cat([
                arange.view(w, 1, 1, 1).repeat([1, w, w, 1]),
                arange.view(1, w, 1, 1).repeat([w, 1, w, 1]),
                arange.view(1, 1, w, 1).repeat([w, w, 1, 1])], dim=-1).unsqueeze(
        ...
        all_indices = torch.cat([
            tiled_batch_indices[:, :num_coords], voxel_indices], -1)

        # BS x N x 4
        voxel_values_pruned_flat = torch.cat(
            [voxel_values, ones_max_coords[:, :num_coords]], -1)
        ...
        if batch_size < self._batch_size:
            scattered = scattered[:batch_size]
        ...
        return torch.cat(
           [vox[..., :-1], index_grid[:, :-2, :-2, :-2] / self._voxel_d,
            vox[..., -1:]], -1)
