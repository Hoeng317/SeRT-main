...
class CustomRLBenchEnv(RLBenchEnv):
    ...
    def __init__(self,
        ...
        self._allow_invalid_actions = bool(allow_invalid_actions)
    ...
    def step(self, act_result: ActResult) -> Transition:
        ...
        try:
            ...
            allow_invalid = self._allow_invalid_actions and not getattr(self, 'eval', False)
            if allow_invalid:
                terminal = False
                reward = 0.0
                try:
                    self._task._scene.step()
                    obs = self.extract_obs(self._task.get_observation())
                    self._previous_obs_dict = obs
                except Exception:
